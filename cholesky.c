...
/* Take the cholesky decomposition in the manner described in FA Graybill (1976).
   This function was originally written by Mark A Abney and changed slightly by
   Johanna Jakobsdottir. Now it allows NULL pointer and zero to be used as input for
   aug and mcol and NULL pointer to be used as output for cholaug.
   
   If **agu = NULL, then the multiplication of the extra matrix by C^-t is not done.
   ...
   Returns with a value of 0 if M is a non-positive definite matrix.
   Returns with a value of 1 with succesful completion.
   
   Arguments:
   
   orig (input) double n x n array. The matrix to take the Cholesky decomposition of.
   n    (input) integer. Number of rows and columns in orig.
   aug  (input) double n x mcol array. The matrix for the augmented part of the decomposition.
   mcol (input) integer. Number of columns in aug.
   chol (output) double n x n array. Holds the upper triangular matrix C on output.
				The lower triangular portion remains unchanged. This maybe the same
				as orig, in which case the upper triangular portion of orig is overwritten.
